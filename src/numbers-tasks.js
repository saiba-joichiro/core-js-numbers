...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const firstLength = Math.sqrt(x1 ** 2 + y1 ** 2);
  const secondLength = Math.sqrt(x2 ** 2 + y2 ** 2);

  const dotProduct = x1 * x2 + y1 * y2;

  const angle = Math.acos((dotProduct / firstLength) * secondLength);
...
function parseNumberFromString(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (
      ...
      // these dumb else if statements here
      integerPart *= baseMultiplier; // because of eslint that forbids
      integerPart += Number(str[i]); // me to use continue statement LOL
    } else if (isFloatingPoint) {
      floatingPointPart += Number(str[i]);
      floatingPointPart /= baseMultiplier;
    }
  }

  const result = integerPart + floatingPointPart;
...
function roundToPowerOfTen(num, pow) {
  ...
  const numberToRound = num % powerOfTen;

  if (powerOfTen - numberToRound > powerOfTen / 2) {
    return num - numberToRound;
  }

  return num + powerOfTen - numberToRound;
}
